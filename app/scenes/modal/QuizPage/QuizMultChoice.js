...
class QuizMultChoice extends Component {
  ...
  answer(){
    if(this.state.selectedOption != null){
      if(this.state.selectedOption.isCorrect){
  ...
  render() {
    return (
      <View>
        ...
        />
        <Text style={{color:'rgb(150, 150, 150)'}}>Selected option: {this.state.selectedOption == null ? 'none' : this.state.selectedOption.text}</Text>
        <Button
          onPress={this.answer}
